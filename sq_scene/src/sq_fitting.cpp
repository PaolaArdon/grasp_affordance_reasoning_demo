...
void SuperquadricFitting::getMinParams(sq_params &param)
{
  param = params_;
}
...
void SuperquadricFitting::preAlign(Eigen::Affine3f &transform, Eigen::Vector3f &variances)
{
  ...
  {/*
    if(pose_est_method_=="iteration")
    {
      ...
      transform_in_eigen = Eigen::Translation3d(pose.position.x, pose.position.y, pose.position.z) * Eigen::Quaternion<double>(pose.orientation.w, pose.orientation.x, pose.orientation.y, pose.orientation.z);
...
void SuperquadricFitting::fit_Param(sq_params& param, double& final_error)
{
